...
const MatchSchedule_PD = (props) => {
  ...
  useEffect(() => {
    const getMatchesByDate = async () => {
      try {
        const response = await axios.get(`https://api.football-data.org/v4/competitions/2014/matches`, {
          headers: {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={{borderBottomWidth: 1, flexDirection:'row'}}>
        ...
        />
        <Text style={{fontSize:20, color:'black', fontWeight:'bold', marginBottom:5}}> 라리가 </Text>
      </View>
...
export default MatchSchedule_PD;
