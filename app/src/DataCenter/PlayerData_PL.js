...
const PlayerData_PL = () => {
  ...
  const [followPlayer, setFollowPlayer] = useState('없음'); //팔로우된 선수 //Son Heung-min, Bruno Fernandes
  ...
  if (selectedTag == '23-24') {
    data = PrimierLeague23_24;
  } else if (selectedTag == '22-23') {
    data = PrimierLeague22_23;
  } else if (selectedTag == '21-22') {
    data = PrimierLeague21_22;
  } else {
    data = ErrorData;
  }

  const filteredData = (searchChange == 'Player') ?  //선수명 혹은 팀명에 따라 데이터 필터
  ...
  const renderItem = ({ item }) => {
    return (
      <TouchableOpacity 
        ...
        onPress={() => router.push(`./PlayerDataDetail?Rk=${item.Rk}&Season=${selectedTag}&League=PL`)}
      >
    ...
    <View style={styles.container}>
      ...
      <View style={styles.pickerContainer}>
        ...
        >
          ...
          <Picker.Item label="23-24시즌 프리미어리그" value="23-24" />
          <Picker.Item label="22-23시즌 프리미어리그" value="22-23" />
          <Picker.Item label="21-22시즌 프리미어리그" value="21-22" />
        </Picker>
      </View>
      ...
      {followData ? //followData가 undefined인 경우 대비
        <TouchableOpacity 
        style={styles.follow}
        onPress={() => router.push(`./PlayerDataDetail?Rk=${followData.Rk}&Season=${selectedTag}&League=PL`)} >
          <View style={styles.row}>
